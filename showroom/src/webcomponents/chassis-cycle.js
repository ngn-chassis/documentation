...
chassis-cycle:not([mode="custom"])) > :not([selected] {
  display: none !important;
     ...
     * @return {Number}
     */get selectedIndex(){for(let a in this.children){if(!this.children.hasOwnProperty(a))continue;let b=this.children.item(a);if("object"==typeof b&&b.hasAttribute("selected"))return parseInt(a)}return null}connectedCallback(){let b=new MutationObserver(a=>{a.forEach(a=>{let{addedNodes:b,removedNodes:c,type:d}=a;switch(d){case"childList":if(0<c.length&&!this.selectedElement)return this.previous();break;default:}})});b.observe(this,{attributes:!1,childList:!0,characterData:!1}),setTimeout(()=>{for(let b in this.children){if(!this.children.hasOwnProperty(b))continue;let c=this.children.item(b);"object"==typeof c&&c!==this.selectedElement&&a.get(this).hideChild(c)}},0)}/**
     * @method hide
     * Deactivate a page.
     * @deprecated
